...
- **2026-03-18** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🎨 **[`paper-illustration`](skills/paper-illustration/SKILL.md)** — 通过 PaperBanana 风格运行时生成论文级架构图和方法示意图，集成到工作流 3（`illustration: ai`；默认浏览器优先，复用独立 Gemini 网页 profile，API 仅作显式备用）。社区贡献 by [@Joseph-li343](https://github.com/Joseph-li343)
...
```bash
# 1. clone 或 fork ARIS，并直接把这个 repo 当工作区使用
...
> | `code review` | `true` | 保持实验 review 闸门开启。设 `false` 可保留旧的“直接实现 → sanity → 部署”路径 |
> | `code review mode` | `debate` | `single` = 单次 reviewer pass，`debate` = 有界多轮“执行者 vs 审查者”辩论循环 |
> | `runtime review` | `true` | sanity 失败或阻塞级运行时异常后重新进入 review |
...
> **想让 Codex 执行、Claude Code 审稿？** 见 [`docs/CODEX_CLAUDE_REVIEW_GUIDE_CN.md`](docs/CODEX_CLAUDE_REVIEW_GUIDE_CN.md)。这条路径会在当前 ARIS 工作区内使用 repo 本地的 `skills/skills-codex/*` 与 `skills/skills-codex-claude-review/*`，并通过本地 `claude-review` MCP bridge 转发 review-heavy skill 的审稿请求。

详见[完整安装指南](#%EF%B8%8F-安装)和[替代模型组合](#-替代模型组合)（无需 Claude/OpenAI API）。

## ✨ 功能亮点

- 📊 **32+ 个可组合 skill** — 自由混搭，或串联为完整流水线（`/idea-discovery`、`/auto-review-loop`、`/paper-writing`、`/research-pipeline`）
...
- 📦 **Repo 本地 vendor skill 暂存** — `tools/aris_skill_manager.py` 先把第三方 skill 装进 `vendor-skills/`，并保持在当前工作区内
...
| 🎨 [`paper-illustration`](skills/paper-illustration/SKILL.md) | 通用 | 基于 PaperBanana 风格运行时生成架构图和方法示意图，集成到工作流 3 |
...
输出 `research/<slug>/IDEA_REPORT.md`（排名后的 idea）+ `research/<slug>/refine-logs/FINAL_PROPOSAL.md`（精炼后的方案）+ `research/<slug>/refine-logs/EXPERIMENT_PLAN.md`（实验路线图）。失败的 idea 也记录在案，避免重复踩坑。

Repo 本地 memory 现在是正式契约的一部分：`/idea-discovery` 会在新一轮搜索 / 再生成前先读 `memory/ideation-memory.md`，并在 top idea 确定、大规模淘汰、review 改变方向后通过 `/research-memory` 回写经验。
...
8. /experiment-bridge → /auto-review-loop → /paper-writing  ← 闭环！
```
...
1. 📋 **解析**实验计划（`research/<slug>/refine-logs/EXPERIMENT_PLAN.md`）
2. 💻 **实现**实验脚本（复用已有代码，加 argparse/logging/seed/checkpoint/auto-resume）
3. ⚔️ **双 AI 辩论循环** — GPT-5.4 先审正确性，包括长任务是否可 resume，再审优化 / 稳定性（`code review mode: debate` 默认开启）
4. ✅ **Sanity + 运行时复审** — 先跑最小实验，写出 `EXPERIMENT_RUNTIME.json` 与 resume 元数据，对长任务做一次 resume smoke test，遇到 OOM / NaN / 输出损坏 / 异常变慢时重新进入 review
5. 🚀 **部署**完整实验到 GPU（`/run-experiment`）
6. 📊 **收集**初始结果与运行时证据，更新实验 tracker

Workflow 1.5 现在也会在重设计实验前先读 `memory/experiment-memory.md`，并且可以复用暂存在 `vendor-skills/` 里的 repo 本地第三方 skill。这些 vendor skill 按设计只留在当前仓库。
...
> 💡 **一键调用：** `/experiment-bridge` 会自动读取当前 active research 下的 `refine-logs/EXPERIMENT_PLAN.md`。也可显式指定：`/experiment-bridge "research/my-topic/refine-logs/EXPERIMENT_PLAN.md"`。

> ⚙️ `CODE_REVIEW`、`CODE_REVIEW_MODE`、`RUNTIME_REVIEW`、`OPTIMIZATION_REVIEW`、`LIGHT_PROFILE`、`LONG_RUN_RESUME`、`LONG_RUN_THRESHOLD`、`AUTO_DEPLOY`、`SANITY_FIRST` 和 `MAX_PARALLEL_RUNS` 均可配置——见[自定义](#%EF%B8%8F-自定义)。
...
>
> **传什么参数？** 简短的主题或范围就够——skill 会自动读取当前 active research 下的叙事文档（`research/<slug>/NARRATIVE_REPORT.md`）、实验结果和历史 review，再结合 repo 级 `memory/` 文件，为 GPT-5.4 组装完整上下文。示例：
...
现在每个关键 round 结束后都会先做一次短 reflection，再通过 `/research-memory "review"` 更新 repo 本地 memory，把 reviewer objection、无效实验模式和已验证有效的修复策略沉淀下来，而不是只留在线程上下文里。
...
- 💾 **上下文压缩恢复** — 每轮结束后持久化状态到 `research/<slug>/refine-logs/REVIEW_STATE.json`。如果上下文窗口满了触发自动 compact，工作流会从状态文件恢复断点继续——无需人工干预
...
**涉及 Skills：** `paper-plan` + `paper-figure` + `paper-illustration` + `paper-write` + `paper-compile` + `auto-paper-improvement-loop` +（投稿后）`paper-poster` + `paper-slides`

> **一键调用：** `/paper-writing "NARRATIVE_REPORT.md"` 会在当前 active research 下自动跑完整个工作流 3。若该工作区缺少 `NARRATIVE_REPORT.md` 但已存在工作流 1.5 / 2 工件，流水线会先自动合成它。

**输入：** 一份 `research/<slug>/NARRATIVE_REPORT.md`，描述研究内容：声明、实验、结果、图表。如果该文件不存在，`paper-writing` 会先从 `research/<slug>/AUTO_REVIEW.md`、proposal / experiment 工件和 `research/<slug>/refine-logs/EXPERIMENT_RUNTIME.json` 自动合成草稿。叙事越详细，尤其是图表描述和定量结果，输出越好。完整示例见 [`templates/NARRATIVE_REPORT_TEMPLATE.md`](templates/NARRATIVE_REPORT_TEMPLATE.md)。

**输出：** 一个可投稿的 `research/<slug>/paper/` 目录，含 LaTeX 源码、干净的 `.bib`（仅含实际引用）、编译好的 PDF。
...
- 🎨 **AI 插图运行时** — hero figure、方法图、架构图走 PaperBanana 风格多阶段链路，并写出 `research/<slug>/figures/illustration_manifest.json`
...
> ⚠️ **图表生成边界：** `/paper-figure` 负责**数据驱动图表**（训练曲线、柱状图、热力图）和 **LaTeX 对比表**（来自 JSON/CSV）。hero figure、架构图、方法图默认交给 `illustration: ai` 的 PaperBanana 风格运行时处理，产出 `research/<slug>/figures/ai_generated/*`、`research/<slug>/figures/illustration_manifest.json` 和更新后的 `research/<slug>/figures/latex_includes.tex`。只有截图、真实照片、qualitative sample grid 这类依赖外部素材的图，才会被标成 `manual_blocker`；需要登录 Gemini 的浏览器状态会记成 `needs_login`；遇到“我不是机器人”/`unusual traffic` 这类人机验证会记成 `needs_human_verification`；其他浏览器/API 运行失败记为 `backend_blocker`。
>
> **AI 作图后端配置**（`illustration: ai`）：默认走浏览器优先，而且现在会自动 bootstrap。首次运行 Workflow 3 或 `paper_illustration_cli.py` 时会自动创建 `.venv`、安装 Playwright、安装 Chromium，并写出 `refine-logs/PAPER_RUNTIME_STATE.json`。运行时会把 Playwright 托管的 Chromium revision 也写进这个状态文件里；如果当前 Playwright 版本要求的 revision 变化了，系统会自动刷新托管 Chromium，显式指定的外部浏览器则不会被自动更新。独立 Gemini automation profile 现在会强制执行严格单窗口预算：重复的空白页、旧 Gemini 页、重复登录页、重复验证页和其他多余页面会被自动清理。如果独立 Gemini profile 还没登录，或者 Google 要求“我不是机器人”/`unusual traffic` 这类人机验证，共享浏览器后端现在会自动拉起或复用一个可交互的 Gemini 窗口并等待恢复；只有等到超时才会返回 `needs_login` 或 `needs_human_verification`，而且这时只会保留唯一必要的恢复页面。只要恢复成功，后端就会自动关掉这个交互窗口，用同一个 dedicated profile 切到 headless Playwright，然后把后续 `status` / `render_image` 放到后台继续执行，不再留下可见的 automation 窗口。每次真实出图前，后端会先把 Gemini 重置到新的临时对话、显式开启“制作图片”、优先选择思考/Pro 模式，只有不可用时才回退到快速/Flash，并在浏览器 prompt 外层自动加一层“只返回图片”的约束，避免 Gemini 回答成文字说明。如果临时对话这条路径持续泄露旧工件，或者没有暴露出可靠的作图入口，重试时会自动升级到 `new_chat`，但仍然复用同一个独立 profile。在没有非交互 `sudo` 的 Linux 环境里，bootstrap 会自动降级为 `python -m playwright install chromium`，不会卡死在 `--with-deps`。若禁用了自动 bootstrap 或 bootstrap 失败，再手动执行 `python3 -m pip install -r mcp-servers/gemini-browser/requirements.txt` 和 `python3 -m playwright install chromium`。若要给 Claude Code 增加插件入口，可执行：`claude mcp add gemini-browser -s user -- python3 /ABS/PATH/TO/Auto-claude-code-research-in-sleep/mcp-servers/gemini-browser/server.py`。只有在显式设置 `ILLUSTRATION_BACKEND=api` 时，才需要配置 `PAPER_ILLUSTRATION_API_KEY` 或 `GEMINI_API_KEY`。可选参考图仍通过 `--reference-dir` 传入；没有参考图时自动降级为无检索模式。

**端到端实测：** 从一份 active research 工作区里的 `NARRATIVE_REPORT.md` 生成了一篇 9 页 ICLR 2026 理论论文（7 节、29 条引用、4 张图、2 个对比表）——零编译错误、零 undefined reference。
...
| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 🔗 **[`experiment-bridge`](skills/experiment-bridge/SKILL.md)** | 读取实验计划 → 实现代码 → 与审查者辩论 → sanity / 运行时复审 → 部署到 GPU → 收集初始结果，并把长任务 resume 能力当成 blocker | 否 |
| ├ 🚀 [`run-experiment`](skills/run-experiment/SKILL.md) | 部署实验到本地（MPS/CUDA）或远程 GPU 服务器，写出 `EXPERIMENT_RUNTIME.json`，并保留长任务 resume 元数据 | 否 |
| └ 👀 [`monitor-experiment`](skills/monitor-experiment/SKILL.md) | 监控实验进度、报告最新 checkpoint / 进度，并告诉你是否可 resume | 否 |
...
| ├ 🚀 [`run-experiment`](skills/run-experiment/SKILL.md) | 部署实验到本地（MPS/CUDA）或远程 GPU 服务器，并写出带 resume 元数据的 `EXPERIMENT_RUNTIME.json` | 否 |
...
| 📦 [`tools/aris_skill_manager.py`](tools/aris_skill_manager.py) | 将第三方 skill 安装到 `vendor-skills/`，支持查看和卸载，并保持扩展只在当前 repo 内生效 | 否 |
...
| ├ 🎨 [`paper-illustration`](skills/paper-illustration/SKILL.md) | PaperBanana 风格 AI 运行时，生成 hero figure、架构图和方法示意图（`illustration: ai` 时启用） | 否（需独立浏览器登录，或 API key 作为备用） |
...
2. （仅 review 类 skill 需要）安装 [Codex CLI](https://github.com/openai/codex) 并配置为 MCP server：
   ...
   claude mcp add codex -s user -- codex mcp-server
   ```
3. （仅工作流 3：论文写作需要）**LaTeX / 作图 / 浏览器运行时**：
   ```bash
   # 默认：首次运行 Workflow 3 时自动 bootstrap
   python3 tools/ensure_paper_runtime.py --phase workflow3

   # 若自动 bootstrap 被禁用或失败，再手动安装：
   # macOS
   ...
   ```
   > 如果只用工作流 1 和 2（找 idea + 自动 review），不需要 Workflow 3 的 bootstrap。
...
ARIS 现在把第三方扩展也收敛为 repo 本地：

```bash
# 把第三方 skill 安装到当前 repo 的 vendor-skills/
...
删除当前 repo 时，`vendor-skills/` 和 `memory/` 会按设计一起消失。
...
| `CODE_REVIEW` | true | 保持实验 review 闸门开启 | → `experiment-bridge` |
| `CODE_REVIEW_MODE` | `debate` | `single` = 单次 pass，`debate` = 有界多轮执行者 vs 审查者辩论循环 | → `experiment-bridge` |
| `RUNTIME_REVIEW` | true | sanity 失败或阻塞级运行时异常后重新进入 review | → `experiment-bridge` |
| `OPTIMIZATION_REVIEW` | true | 第二个 pass 审查后端 / 显存 / 吞吐优化机会 | → `experiment-bridge` |
| `WORKLOAD_PROFILE` | `mixed` | 提示当前负载更偏训练、推理或混合 | → `experiment-bridge` |
...
| `PAPER_VENV_DIR` | `.venv` | Workflow 3 使用的项目级虚拟环境 | → `paper-writing` |
| `PAPER_SYSTEM_INSTALL` | `auto` | 通过 `apt-get` 或 `brew` 自动安装支持的系统包 | → `paper-writing` → `paper-compile` |

行内覆盖：`/research-pipeline "方向" — auto proceed: false, wandb: true, illustration: ai`
...
| 常量 | 默认值 | 说明 |
|------|--------|------|
| `CODE_REVIEW` | true | 保持实验 review 闸门开启。设 `false` 可回退到旧的“直接实现 → sanity → 部署”路径 |
| `CODE_REVIEW_MODE` | `debate` | `single` = 单次 reviewer pass，`debate` = 有界多轮执行者 vs 审查者辩论循环 |
| `MAX_DEBATE_ROUNDS` | 3 | 最多预运行辩论轮数；若关键阻塞未清除则停止自动部署 |
| `RUNTIME_REVIEW` | true | sanity 失败或阻塞级运行时异常后重新进入 review |
| `OPTIMIZATION_REVIEW` | true | 第二个 pass 审查框架 / 后端 / 显存 / 吞吐优化机会 |
| `OPTIMIZATION_AUTHORITY` | `recommend` | v1 仅建议，不允许静默切换框架或内核实现 |
| `WORKLOAD_PROFILE` | `mixed` | 提示当前负载更偏 `training`、`inference` 或 `mixed` |
| `LIGHT_PROFILE` | true | sanity 时在支持情况下请求短时热点 / 显存采样；否则回退到粗粒度运行时证据 |
| `LONG_RUN_RESUME` | `required` | 长任务必须 checkpoint 并 auto-resume；缺 resume 能力会阻塞部署 |
| `LONG_RUN_THRESHOLD` | `10min_or_multi_step` | 如果任务明显是迭代式的，或预计超过约 10 分钟，就按长任务处理 |
...
| `TARGET_VENUE` | `ICLR` | 目标会议格式：`ICLR`、`NeurIPS`、`ICML`、`CVPR`、`ACL`、`AAAI`、`ACM` |
| `ANONYMOUS` | true | 匿名审稿模式 |
| `MAX_PAGES` | 9 | 正文页数上限（不含参考文献） |
| `ILLUSTRATION` | `ai` | AI 作图：`ai`（默认，浏览器优先的 Gemini 网页端渲染）、`mermaid`（免费）、`false`（跳过） |
| `ILLUSTRATION_BACKEND` | `browser` | `illustration: ai` 的后端：`browser`（默认）或 `api`（显式备用） |
| `PAPER_AUTO_INSTALL` | true | 在 figure / write / compile 前自动 bootstrap Workflow 3 依赖 |
| `PAPER_VENV_DIR` | `.venv` | Workflow 3 Python 工具使用的项目级虚拟环境 |
| `PAPER_SYSTEM_INSTALL` | `auto` | Workflow 3 需要时自动安装支持的系统包 |

行内覆盖：`/paper-write — target venue: NeurIPS, illustration: ai`
