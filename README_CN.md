...
- 🖥️ **审稿驱动实验** — `experiment-bridge` 现在会在部署前运行有界的“执行者 vs 审查者”辩论循环，写出 `EXPERIMENT_RUNTIME.json`，并在阻塞级运行时失败后重新进入 review；成功实验仍会自动折回论文。只需在 `CLAUDE.md` 里配好服务器信息（[配置指南](#%EF%B8%8F-gpu-服务器配置自动实验用)）
...
3. ⚔️ **双 AI 辩论循环** — GPT-5.4 先审正确性，再审优化 / 稳定性（`code review mode: debate` 默认开启）
4. ✅ **Sanity + 运行时复审** — 先跑最小实验，写出 `EXPERIMENT_RUNTIME.json`，遇到 OOM / NaN / 输出损坏 / 异常变慢时重新进入 review
...
> ⚙️ `CODE_REVIEW`、`CODE_REVIEW_MODE`、`RUNTIME_REVIEW`、`OPTIMIZATION_REVIEW`、`LIGHT_PROFILE`、`AUTO_DEPLOY`、`SANITY_FIRST` 和 `MAX_PARALLEL_RUNS` 均可配置——见[自定义](#%EF%B8%8F-自定义)。
...
| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 🔗 **[`experiment-bridge`](skills/experiment-bridge/SKILL.md)** | 读取实验计划 → 实现代码 → 与审查者辩论 → sanity / 运行时复审 → 部署到 GPU → 收集初始结果 | 否 |
| ├ 🚀 [`run-experiment`](skills/run-experiment/SKILL.md) | 部署实验到本地（MPS/CUDA）或远程 GPU 服务器，并写出 `EXPERIMENT_RUNTIME.json` | 否 |
