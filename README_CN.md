...
> /paper-writing "NARRATIVE_REPORT.md"       # 工作流 3：研究叙事（或自动合成 handoff）→ 精修 PDF
...
> **一键调用：** `/paper-writing "NARRATIVE_REPORT.md"` 自动跑完整个工作流 3。若缺少 `NARRATIVE_REPORT.md` 但已存在工作流 1.5 / 2 工件，流水线会先自动合成它。

**输入：** 一份 `NARRATIVE_REPORT.md`，描述研究内容：声明、实验、结果、图表。如果该文件不存在，`paper-writing` 会先从 `AUTO_REVIEW.md`、proposal / experiment 工件和 `EXPERIMENT_RUNTIME.json` 自动合成草稿。叙事越详细（尤其是图表描述和定量结果），输出越好。完整示例见 [`templates/NARRATIVE_REPORT_TEMPLATE.md`](templates/NARRATIVE_REPORT_TEMPLATE.md)。
...
- 🎨 **AI 插图运行时** — hero figure、方法图、架构图走 PaperBanana 风格多阶段链路，并写出 `figures/illustration_manifest.json`
...
> ⚠️ **图表生成边界：** `/paper-figure` 负责**数据驱动图表**（训练曲线、柱状图、热力图）和 **LaTeX 对比表**（来自 JSON/CSV）。hero figure、架构图、方法图默认交给 `illustration: ai` 的 PaperBanana 风格运行时处理，产出 `figures/ai_generated/*`、`figures/illustration_manifest.json` 和更新后的 `figures/latex_includes.tex`。只有截图、真实照片、qualitative sample grid 这类依赖外部素材的图，才会被标成 `manual_blocker`；需要登录 Gemini 的浏览器状态会记成 `needs_login`；遇到“我不是机器人”/`unusual traffic` 这类人机验证会记成 `needs_human_verification`；其他浏览器/API 运行失败记为 `backend_blocker`。
>
