...
Workflow 1.5 现在也会在重设计实验前先读 `memory/experiment-memory.md`，并且可以复用暂存在 `vendor-skills/` 里的 repo 本地第三方 skill。除非你显式运行 `python3 tools/aris_skill_manager.py sync-global --target auto`，这些 vendor skill 都只留在当前仓库。
...
ARIS 现在支持先在当前仓库里暂存第三方 skill，不用一上来就改全局目录：
