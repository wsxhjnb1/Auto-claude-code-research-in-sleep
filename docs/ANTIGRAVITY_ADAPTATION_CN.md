...
| ARIS 工作区 | 在 ARIS repo 中直接使用 `skills/.../SKILL.md` | 打开 ARIS repo 作为 Antigravity 工作区，并读取 repo 内 `skills/.../SKILL.md` |
...
> **重要：** ARIS 现在只支持 **repo 工作区模式**。`skills/`、`tools/`、`memory/`、`vendor-skills/`、`refine-logs/` 都应保留在这个克隆仓库中，并由 Antigravity 直接围绕该工作区运行。

如果你的 Antigravity 版本为了技能发现需要显式导入文件，也只导入 **这个 repo 中的 `SKILL.md`**，继续维护这里的原文件即可。
...
- 关键文件: `research/<slug>/NARRATIVE_REPORT.md`, `research/<slug>/IDEA_REPORT.md`
```
...
如果当前工作区就是 ARIS repo（见 §3.1），Antigravity 会从 repo 内 `skills/` 自动发现并激活 `auto-review-loop` 等技能。
...
输入：active research 工作区里的 `research/<slug>/NARRATIVE_REPORT.md`。
...
| 阶段 | 执行方式 | 产出文件 |
|------|---------|---------|
| 1 | 创意发现：`skills/idea-discovery/SKILL.md` + 研究方向 | `research/<slug>/IDEA_REPORT.md`, `research/<slug>/refine-logs/FINAL_PROPOSAL.md`, `research/<slug>/refine-logs/EXPERIMENT_PLAN.md` |
| 2 | 实验桥接：`skills/experiment-bridge/SKILL.md` | 实验脚本、`research/<slug>/results/`、`research/<slug>/refine-logs/EXPERIMENT_RUNTIME.json` |
| 3 | 自动评审：`skills/auto-review-loop/SKILL.md` | `research/<slug>/AUTO_REVIEW.md` |
| 4 | 论文写作：`skills/paper-writing/SKILL.md` + `research/<slug>/NARRATIVE_REPORT.md`（若已有工作流 2 工件则可省略） | `research/<slug>/paper/` 目录 |

> **说明：** 第 4 阶段最好有 `research/<slug>/NARRATIVE_REPORT.md`，但更新后的 `paper-writing` 已支持在文件缺失时，自动从 `research/<slug>/AUTO_REVIEW.md`、proposal / experiment 工件和运行时证据合成它。目标格式可参考 [NARRATIVE_REPORT_EXAMPLE.md](NARRATIVE_REPORT_EXAMPLE.md)。
...
| 文件 | 作用 | 对应流程 |
|------|------|---------|
| `research/<slug>/refine-logs/REVIEW_STATE.json` | 自动评审进度 | auto-review-loop |
| `research/<slug>/AUTO_REVIEW.md` | 累计评审日志 | auto-review-loop |
| `research/<slug>/IDEA_REPORT.md` | 创意筛选与排名 | idea-discovery |
| `research/<slug>/PAPER_PLAN.md` | 论文大纲 + claim-evidence matrix | paper-plan |
| `research/<slug>/refine-logs/FINAL_PROPOSAL.md` | 精化后的方法提案 | research-refine |
| `research/<slug>/refine-logs/EXPERIMENT_PLAN.md` | 实验路线图 | experiment-plan |

中断恢复示例：

```
读取 skills/auto-review-loop/SKILL.md，然后读取 `research/<slug>/refine-logs/REVIEW_STATE.json` 和 `research/<slug>/AUTO_REVIEW.md`。
从保存的状态恢复自动评审循环。
```
...
- `research/<slug>/IDEA_REPORT.md` → implementation plan artifact
- `research/<slug>/AUTO_REVIEW.md` → walkthrough artifact
- `research/<slug>/PAPER_PLAN.md` → implementation plan artifact
...
# 论文写作
读取 skills/paper-writing/SKILL.md，根据 `research/<slug>/NARRATIVE_REPORT.md` 写论文。
...
- [ ] 打开 ARIS repo 作为当前 Antigravity 工作区
- [ ] 直接读取 repo 内 `skills/.../SKILL.md`，不要复制到其他目录
