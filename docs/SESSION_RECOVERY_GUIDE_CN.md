...
> 在 ARIS 工作流中跨会话和上下文压缩维护项目状态 — 核心设计是 **`research/<slug>/CLAUDE.md` 中的 Pipeline Status**，repo 根 `CLAUDE.md` 只作共享默认值，Claude Code 用户可选 hook 自动化。
...
最重要的一件事是在 research workspace 的 `CLAUDE.md`（`research/<slug>/CLAUDE.md`）中维护一个 **`## Pipeline Status`** 段。这是一个轻量级、结构化的项目快照——30 秒读完，足以让任何 LLM 恢复工作。repo 根 `CLAUDE.md` 若存在，只应该放服务器、Paper Library 之类的共享默认值。

### Pipeline Status 包含什么

在 `research/<slug>/CLAUDE.md` 中添加：
...
在 `research/<slug>/CLAUDE.md` 中添加这些规则，让 LLM 知道何时以及如何维护状态：
