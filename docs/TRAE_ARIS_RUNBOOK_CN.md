...
> **提示：** 如果上下文过长，可以将每个阶段拆分为单独的对话，通过当前 active research 工作区里的文件（如 `research/<slug>/IDEA_REPORT.md`、`research/<slug>/refine-logs/FINAL_PROPOSAL.md`）传递结果。
...
```
使用 paper-writing 技能。
输入：active research 工作区里的 `research/<slug>/NARRATIVE_REPORT.md`；如果缺失，也可以让更新后的工作流先从工作流 2 工件自动合成。
...
| 阶段 | 执行方式 | 产出文件 |
|------|---------|---------|
| 1 | 创意发现：使用 `idea-discovery` 技能 + 研究方向 | `research/<slug>/IDEA_REPORT.md`, `research/<slug>/refine-logs/FINAL_PROPOSAL.md`, `research/<slug>/refine-logs/EXPERIMENT_PLAN.md` |
| 2 | 实验桥接：使用 `experiment-bridge` 技能 | 实验脚本、`research/<slug>/results/`、`research/<slug>/refine-logs/EXPERIMENT_RUNTIME.json` |
| 3 | 自动评审：使用 `auto-review-loop` 技能 | `research/<slug>/AUTO_REVIEW.md` |
| 4 | 论文写作：使用 `paper-writing` 技能 + `research/<slug>/NARRATIVE_REPORT.md` | `research/<slug>/paper/` 目录 |
...
| 文件 | 作用 | 典型流程 |
|---|---|---|
| `research/<slug>/refine-logs/REVIEW_STATE.json` | 记录自动审阅进度 | auto-review-loop |
| `research/<slug>/AUTO_REVIEW.md` | 累计审阅日志 | auto-review-loop |
| `research/<slug>/IDEA_REPORT.md` | 创意筛选与初评结果 | idea-discovery |
| `research/<slug>/PAPER_PLAN.md` | 论文大纲与 claim-evidence matrix | paper-plan |
| `research/<slug>/paper/PAPER_IMPROVEMENT_LOG.md` | 论文改进回合日志 | auto-paper-improvement-loop |
...
# 论文写作
使用 paper-writing 技能，根据 `research/<slug>/NARRATIVE_REPORT.md` 写论文。
