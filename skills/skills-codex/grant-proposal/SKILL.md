...
**State**: Write `$RESEARCH_ROOT/grant-proposal/GRANT_STATE.json` with `phase: 2`, aims summary, and the reviewer agent id.
...
Save the returned reviewer agent id in `$RESEARCH_ROOT/grant-proposal/GRANT_STATE.json` if you want to continue the same dialogue in Round 2.
