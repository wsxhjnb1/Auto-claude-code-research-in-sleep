...
Continue on success or "no updates". If the sync reports only a temporary fetch / network failure, note it and continue. If it reports tracked worktree changes, local `main` vs `origin/main` divergence, or an unresolved sync conflict, stop and clean up the repo state first. The sync flow is origin-first: update local `main` from `origin/main` when possible, then inspect `upstream/main`.
